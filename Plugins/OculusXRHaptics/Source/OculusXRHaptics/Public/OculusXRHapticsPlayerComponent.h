...
#include "OculusXRHapticsPlayerComponent.generated.h"

class UOculusXRHapticClip;
class FOculusXRHapticsModule;

UENUM(BlueprintType)
enum class EOculusXRHapticController : uint8
{
 ...
 */
UCLASS(ClassGroup = (OculusXRHaptics), meta = (BlueprintSpawnableComponent, DisplayName = "OculusXR Haptics Player Component"))
class OCULUSXRHAPTICS_API UOculusXRHapticsPlayerComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	UOculusXRHapticsPlayerComponent();

	/**
	 * Start playback of the haptic clip set with the HapticClip property.
	 */
	UFUNCTION(BlueprintCallable, Category = "OculusXR|Haptics")
	void Play();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "OculusXR|Haptics")
	void PlayOnController(const EOculusXRHapticController InController);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "OculusXR|Haptics")
	void PlayWithInputs(const EOculusXRHapticController InController, const int32 InPriority = 512,
	...
	UFUNCTION(BlueprintPure, BlueprintInternalUseOnly, Category = "OculusXR|Haptics")
	UOculusXRHapticClip* GetHapticClip() const;

	UFUNCTION(BlueprintCallable, BlueprintInternalUseOnly, Category = "OculusXR|Haptics")
	void SetHapticClip(UOculusXRHapticClip* InHapticClip);

	UFUNCTION(BlueprintPure, BlueprintInternalUseOnly, Category = "OculusXR|Haptics")
	EOculusXRHapticController GetController() const;

	UFUNCTION(BlueprintCallable, BlueprintInternalUseOnly, Category = "OculusXR|Haptics")
	void SetController(const EOculusXRHapticController& InController);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "OculusXR|Haptics")
	float GetClipDuration() const;

	/**
	 * Internal method used by UOculusXRHapticsFunctionLibrary, do not call.
	 *
	 * Used to set initial values when spawning a new component from code. Needs to be called before
	 * BeginPlay().
	 */
	void SetInitialValues(UOculusXRHapticClip* InHapticClip, const EOculusXRHapticController InController, const int32 InPriority = 512,
		const float InAmplitude = 1.0f, const float InFrequencyShift = 0.0f, const bool bInIsLooping = false);
...
private:
	...
	UPROPERTY(EditAnywhere, BlueprintGetter = GetHapticClip, BlueprintSetter = SetHapticClip,
		Category = "OculusXR|Haptics")
	UOculusXRHapticClip* HapticClip = nullptr;

	/**
	 * The controller on which the haptic clip will be played back.
	 */
	UPROPERTY(EditAnywhere, BlueprintGetter = GetController, BlueprintSetter = SetController, Category = "OculusXR|Haptics")
	EOculusXRHapticController Controller = EOculusXRHapticController::Both;
	...
	UPROPERTY(EditAnywhere, BlueprintGetter = GetPriority, BlueprintSetter = SetPriority,
		Category = "OculusXR|Haptics", meta = (ClampMin = "0", ClampMax = "1024", UIMin = "0", UIMax = "1024"))
	int32 Priority = 512;
	...
	UPROPERTY(EditAnywhere, BlueprintGetter = GetAmplitude, BlueprintSetter = SetAmplitude,
		Category = "OculusXR|Haptics", meta = (ClampMin = "0", UIMin = "0"))
	...
	UPROPERTY(EditAnywhere, BlueprintGetter = GetFrequencyShift, BlueprintSetter = SetFrequencyShift,
		Category = "OculusXR|Haptics", meta = (ClampMin = "-1.0", ClampMax = "1.0", UIMin = "-1.0", UIMax = "1.0"))
	...
	FOculusXRHapticsModule* HapticsModule = nullptr;
