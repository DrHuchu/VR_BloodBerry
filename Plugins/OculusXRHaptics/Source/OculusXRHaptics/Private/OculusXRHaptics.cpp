...
OCULUSXRHAPTICS_API DEFINE_LOG_CATEGORY(LogHapticsSDK);

#define LOCTEXT_NAMESPACE "FOculusXRHapticsModule"

template <typename Func>
Func FOculusXRHapticsModule::LoadFunction(const FString& Name)
{
...
FOculusXRHapticsModule* FOculusXRHapticsModule::GetIfLibraryLoaded()
{
	if (!FModuleManager::Get().IsModuleLoaded("OculusXRHaptics"))
	{
		UE_LOG(LogHapticsSDK, Error, TEXT("Haptics SDK module is not available"));
		return nullptr;
	}

	FOculusXRHapticsModule* const OculusModule = &Get();
	if (OculusModule->HapticsSDKLibraryHandle == nullptr)
	{
		return nullptr;
	}
...
void FOculusXRHapticsModule::StartupModule()
{
#if PLATFORM_WINDOWS
	const FString BaseDir = IPluginManager::Get().FindPlugin(TEXT("OculusXRHaptics"))->GetBaseDir();
...
void FOculusXRHapticsModule::ShutdownModule()
{
	if (HapticsSDKLibraryHandle == nullptr)
	{
		return;
...
IMPLEMENT_MODULE(FOculusXRHapticsModule, OculusXRHaptics)
